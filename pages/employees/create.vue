        ...
        <form @submit.prevent="saveEmployee">
          ...
          <div class="mb-3">
            ...
            <!-- <div class="input-group mb-3">
              <select class="form-select" id="selectedGender" v-model="employee.gender"
                :class="{ 'is-invalid': errors.gender && errors.gender.length > 0 }">
                <option selected></option>
                <option value="1">Laki-laki</option>
                <option value="2">Perempuan</option>
              </select>
              <div v-if="errors.gender && errors.gender.length > 0" class="invalid-feedback">
                {{ errors.gender[0] }}
              </div>
            </div> -->
            <div class="form-check">
              <input class="form-check-input" type="radio" id="genderMale" value="Laki-laki" v-model="employee.gender"
                :class="{ 'is-invalid': errors.gender && errors.gender.length > 0 }">
            ...
            <div class="form-check">
              <input class="form-check-input" type="radio" id="genderFemale" value="Perempuan" v-model="employee.gender"
                :class="{ 'is-invalid': errors.gender && errors.gender.length > 0 }">
...
export default {
  ...
  watch: {
    'employee.nip': function (newVal) {
      this.employee.nip = newVal.toString();
    },
    'employee.phone_number': function (newVal) {
      this.employee.phone_number = newVal.toString();
    },
    'employee.npwp': function (newVal) {
      this.employee.npwp = newVal.toString();
    }
  },
  mounted() {
  ...
  methods: {
    saveEmployee() {
      ...
      axios.post(`http://localhost:8000/api/employee`, this.employee)
        .then(res => {
        ...
        .catch(error => {
          ...
          if (error.response.data.status == 422) {
            ...
            console.log(this.errors.gender[0]);
          } else {
            this.errors = {
              birth_date: [],
              gender: [],
